...
- `docs/adr/0001-0008`: format (Zarr v3 + GeoZarr), access (zarrita + icechunk-js), dual chunk layout with ZEP0005-aligned stats and sparse pyramid spacing, compute-plane spectral math with derived-tile rendering, Mosaic + DuckDB-WASM, two-stage prism-core split, display projection model, numerical policy for streaming fits. Do not relitigate accepted ADRs in passing; propose a superseding ADR if evidence demands it.
